...
export default function MenuBuilder({ restaurant }: Props) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
              ...
              <div style={{
                ...
                {CATEGORY_EMOJIS.map(e => (
                  <button key={e} type="button" onClick={() => { setNewCatIcon(e); setShowEmojiPicker(false); }}
                    style={{ fontSize: 20, background: newCatIcon === e ? "#fef3c7" : "none", border: "none", borderRadius: 6, cursor: "pointer", padding: 4, lineHeight: 1 }}>
                    {e}
                  </button>
                ))}
              </div>
        ...
        <div key={cat.id} className="card">
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {items.filter(i => i.category_id === cat.id).map(item => (
              <div key={item.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 12px", background: item.is_available ? "#f9fafb" : "#fef2f2", borderRadius: 8, gap: 8, flexWrap: "wrap" }}>
            ...
            {items.filter(i => i.category_id === cat.id).length === 0 && (
              <div style={{ textAlign: "center", padding: "24px 16px", color: "var(--text-muted)", background: "#f9fafb", borderRadius: 8 }}>
      ...
      {categories.length === 0 && (
        <div style={{ textAlign: "center", padding: "48px 16px", color: "var(--text-muted)", background: "#f9fafb", borderRadius: 14, border: "2px dashed var(--border)" }}>
          <div style={{ fontSize: 52, marginBottom: 12 }}>🍽️</div>
          <p style={{ fontWeight: 600, fontSize: 15, color: "#374151", marginBottom: 4 }}>Your menu is empty</p>
