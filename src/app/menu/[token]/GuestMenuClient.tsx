...
export default function GuestMenuClient({ table, restaurant, categories, items }: Props) {
    ...
    return (
      <div style={{ minHeight: "100vh", background: accentColor, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 32, fontFamily: "Inter, system-ui, sans-serif" }}>
        ...
        <button
          ...
          style={{ padding: "14px 36px", borderRadius: 14, background: "white", color: accentColor, border: "none", fontWeight: 800, fontSize: 16, cursor: "pointer", animation: "fadeUp 0.4s ease 0.5s both", boxShadow: "0 4px 20px rgba(0,0,0,0.15)" }}
        >
          Back to menu
        </button>
      </div>
  ...
  // --- MAIN MENU ---
  return (
    <div style={{ minHeight: "100vh", background: "#FAFAF8", fontFamily: "Inter, system-ui, sans-serif", paddingBottom: sessionRequests.length > 0 ? 110 : 90 }}>
      <style>{`
        ...
        <div role="group" aria-label="Quick actions" style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
          {([
            ...
            <button key={type} onClick={() => sendRequest(type)}
              style={{ padding: "16px 8px", borderRadius: 14, border: `2px solid ${accentColor}`, background: "white", cursor: "pointer", textAlign: "center", display: "flex", flexDirection: "column", alignItems: "center", gap: 5, WebkitTapHighlightColor: "transparent", boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}
      ...
      {items.length === 0 && (
        <div style={{ textAlign: "center", padding: "48px 24px", color: "#6b7280" }}>
          ...
          <h2 style={{ fontWeight: 700, fontSize: 20, color: "#374151", marginBottom: 8 }}>Menu coming soon</h2>
          <p style={{ fontSize: 14, color: "#9ca3af" }}>The restaurant is still setting up their menu. Please ask your server.</p>
        </div>
      )}
      ...
      {sessionRequests.length > 0 && (
        <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 30 }}>
          <button
            ...
            style={{ width: "100%", background: "#1f2937", color: "white", border: "none", padding: "12px 16px", display: "flex", alignItems: "center", justifyContent: "space-between", cursor: "pointer", fontSize: 14, fontWeight: 700 }}
          >
            <span>🧾 Your requests ({sessionRequests.reduce((s, r) => s + r.qty, 0)} items)</span>
            <span style={{ fontSize: 18 }}>{sessionPanelOpen ? "▼" : "▲"}</span>
          </button>
          {sessionPanelOpen && (
            <div style={{ background: "white", borderTop: "1px solid #e5e7eb", padding: "12px 16px", maxHeight: 200, overflowY: "auto" }}>
