  ...
  return (
    <main style={{ minHeight: "100vh", background: "var(--bg)", fontFamily: "var(--font-body)" }}>
      ...
      <nav style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "16px 32px", background: "var(--surface)", borderBottom: "1px solid var(--border)", position: "sticky", top: 0, zIndex: 100 }}>
        <span style={{ fontWeight: 800, fontSize: "var(--fs-xl)", color: "var(--accent)", letterSpacing: "-0.5px" }}>MenuQR</span>
        <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
          <Link href="/login" style={{ padding: "8px 18px", borderRadius: "var(--radius-md)", border: "1px solid var(--border)", color: "var(--text)", textDecoration: "none", fontWeight: 500, fontSize: "var(--fs-sm)" }}>Log in</Link>
          <Link href="/signup" style={{ padding: "8px 18px", borderRadius: "var(--radius-md)", background: "var(--accent)", color: "white", textDecoration: "none", fontWeight: 700, fontSize: "var(--fs-sm)" }}>Get started free</Link>
        </div>
      </nav>
      ...
      <section style={{ maxWidth: 1040, margin: "0 auto", padding: "72px 32px 64px", display: "flex", gap: 48, alignItems: "center", flexWrap: "wrap", justifyContent: "center" }}>
        <div style={{ flex: "1 1 420px", maxWidth: 560 }}>
          ...
          <p style={{ fontSize: "var(--fs-lg)", color: "var(--text-muted)", margin: "0 0 32px", lineHeight: 1.6, maxWidth: 480 }}>
          ...
          <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
            ...
            <Link href="#how" style={{ display: "inline-block", padding: "14px 24px", borderRadius: "var(--radius-md)", border: "1px solid var(--border)", color: "var(--text)", textDecoration: "none", fontWeight: 600, fontSize: "var(--fs-md)" }}>See how it works</Link>
          </div>
          <p style={{ marginTop: 16, fontSize: "var(--fs-sm)", color: "var(--text-muted)" }}>Free forever • No credit card</p>
        </div>
        ...
        <div aria-hidden="true" style={{ flex: "0 0 auto", width: 260, borderRadius: 36, border: "1px solid var(--border)", background: "var(--surface)", boxShadow: "var(--shadow-float)", padding: 12 }}>
          <div style={{ borderRadius: 26, overflow: "hidden", border: "1px solid var(--border)", background: "var(--bg)" }}>
            ...
            <div style={{ background: "var(--accent)", padding: "18px 16px 14px" }}>
              ...
              <div style={{ color: "rgba(255,255,255,0.8)", fontSize: "var(--fs-xs)", marginTop: 2 }}>Table 4 · Menu</div>
            </div>
            {/* Category pills */}
            ...
            {[
              ...
              <div key={name} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, margin: "8px 12px 0", padding: "10px 12px", background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "var(--radius-lg)" }}>
                <div>
                  ...
                  <div style={{ fontSize: "var(--fs-xs)", fontWeight: 700, color: "var(--accent)", marginTop: 2 }}>{price}</div>
                </div>
      ...
      <section id="how" style={{ maxWidth: 880, margin: "0 auto", padding: "24px 32px 64px" }}>
        ...
        <p style={{ textAlign: "center", color: "var(--text-muted)", marginBottom: 40, fontSize: "var(--fs-md)" }}>No hardware. No complex setup. Just your menu, online.</p>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 20 }}>
          {steps.map((s) => (
            <div key={s.title} className="feature-card">
              <div style={{ width: 28, height: 28, borderRadius: "50%", background: "var(--accent)", color: "white", fontSize: "var(--fs-sm)", fontWeight: 800, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 12 }}>{s.step}</div>
              <h3 style={{ fontWeight: 700, marginBottom: 6, fontSize: "var(--fs-md)", color: "var(--text)" }}>{s.title}</h3>
              <p style={{ color: "var(--text-muted)", fontSize: "var(--fs-sm)", lineHeight: 1.5, margin: 0 }}>{s.text}</p>
            </div>
          ))}
        </div>
      </section>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 20 }}>
            {features.map(({ Icon, title, text }) => (
              <div key={title} className="feature-card" style={{ borderTop: "3px solid var(--accent)" }}>
                ...
                <h3 style={{ fontWeight: 700, marginBottom: 6, fontSize: "var(--fs-md)", color: "var(--text)" }}>{title}</h3>
                <p style={{ color: "var(--text-muted)", fontSize: "var(--fs-sm)", lineHeight: 1.5, margin: 0 }}>{text}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="pricing" style={{ background: "var(--surface)", borderTop: "1px solid var(--border)", borderBottom: "1px solid var(--border)", padding: "64px 32px" }}>
        <div style={{ maxWidth: 720, margin: "0 auto" }}>
          ...
          <p style={{ textAlign: "center", color: "var(--text-muted)", marginBottom: 40, fontSize: "var(--fs-md)" }}>Start free. Upgrade later if you need more.</p>
          <div style={{ display: "flex", gap: 20, flexWrap: "wrap", justifyContent: "center" }}>
            <div style={{ flex: "1 1 260px", background: "var(--bg)", border: "2px solid var(--accent)", borderRadius: "var(--radius-xl)", padding: "28px 24px" }}>
              ...
              <div style={{ fontSize: "var(--fs-sm)", color: "var(--text-muted)", marginBottom: 20 }}>forever</div>
              <ul style={{ listStyle: "none", padding: 0, margin: "16px 0 24px", display: "flex", flexDirection: "column", gap: 10 }}>
                {["1 restaurant", "Up to 10 tables", "Unlimited menu items", "Live orders dashboard", "QR code generation", "Analytics overview", "Dark mode & multi-currency"].map((f) => (
                  <li key={f} style={{ fontSize: "var(--fs-sm)", color: "var(--text)", display: "flex", gap: 8, alignItems: "flex-start" }}>
                    <IconCheck width={15} height={15} style={{ color: "var(--success)", flexShrink: 0, marginTop: 2 }} /> {f}
                  </li>
                ))}
              </ul>
              <Link href="/signup" style={{ display: "block", textAlign: "center", padding: "12px", borderRadius: "var(--radius-md)", background: "var(--accent)", color: "white", textDecoration: "none", fontWeight: 700, fontSize: "var(--fs-md)" }}>Get started free</Link>
            </div>
            <div style={{ flex: "1 1 260px", background: "var(--bg)", border: "1px solid var(--border)", borderRadius: "var(--radius-xl)", padding: "28px 24px", opacity: 0.6 }}>
              <div style={{ fontWeight: 800, fontSize: "var(--fs-lg)", color: "var(--text)", marginBottom: 6 }}>Pro</div>
              <div style={{ fontSize: "var(--fs-xl)", fontWeight: 900, color: "var(--text-muted)", marginBottom: 4 }}>Coming soon</div>
              <ul style={{ listStyle: "none", padding: 0, margin: "24px 0 0", display: "flex", flexDirection: "column", gap: 10 }}>
                {["Unlimited tables", "Multiple locations", "Custom domain", "Multi-language menus", "Priority support"].map((f) => (
                  <li key={f} style={{ fontSize: "var(--fs-sm)", color: "var(--text-muted)", display: "flex", gap: 8, alignItems: "flex-start" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {faqs.map((f) => (
            <details key={f.q} className="faq-item">
              <summary>{f.q}</summary>
              <p style={{ color: "var(--text-muted)", fontSize: "var(--fs-sm)", lineHeight: 1.6, margin: "12px 0 0" }}>{f.a}</p>
            </details>
          ))}
        </div>
      </section>
      ...
      <section style={{ background: "var(--accent)", padding: "64px 32px", textAlign: "center" }}>
        ...
        <p style={{ color: "rgba(255,255,255,0.85)", fontSize: "var(--fs-md)", marginBottom: 32 }}>Set up your menu tonight, take orders tomorrow. Free to start, no credit card needed.</p>
        <Link href="/signup" style={{ display: "inline-block", padding: "16px 40px", borderRadius: "var(--radius-lg)", background: "white", color: "var(--accent)", textDecoration: "none", fontWeight: 800, fontSize: "var(--fs-lg)" }}>
      ...
      <footer style={{ borderTop: "1px solid var(--border)", background: "var(--surface)", padding: "48px 32px 32px" }}>
        <div style={{ maxWidth: 880, margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 32 }}>
          <div>
            ...
            <p style={{ color: "var(--text-muted)", fontSize: "var(--fs-sm)", lineHeight: 1.6, margin: 0 }}>QR code menus and live table ordering for restaurants and cafés.</p>
          </div>
          <div>
            <div style={{ fontWeight: 700, fontSize: "var(--fs-sm)", color: "var(--text)", marginBottom: 12, textTransform: "uppercase", letterSpacing: "0.5px" }}>Product</div>
          ...
          <div>
            <div style={{ fontWeight: 700, fontSize: "var(--fs-sm)", color: "var(--text)", marginBottom: 12, textTransform: "uppercase", letterSpacing: "0.5px" }}>Account</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          ...
          <div>
            <div style={{ fontWeight: 700, fontSize: "var(--fs-sm)", color: "var(--text)", marginBottom: 12, textTransform: "uppercase", letterSpacing: "0.5px" }}>Legal</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        ...
        <p style={{ maxWidth: 880, margin: "32px auto 0", paddingTop: 20, borderTop: "1px solid var(--border)", color: "var(--text-muted)", fontSize: "var(--fs-sm)" }}>© 2026 MenuQR. All rights reserved.</p>
      </footer>
    </main>
  );
}
