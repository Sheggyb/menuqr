...
function getPasswordStrength(pw: string): { level: number; labelKey: TKey | null; color: string } {
  if (pw.length === 0) return { level: 0, labelKey: null, color: "#e5e7eb" };
  let score = 0;
  if (pw.length >= 8) score++;
  if (/[A-Z]/.test(pw)) score++;
  if (/[0-9]/.test(pw)) score++;
  if (/[^A-Za-z0-9]/.test(pw)) score++;
  if (score <= 1) return { level: 1, labelKey: "auth.signup.strengthWeak", color: "#ef4444" };
  if (score === 2) return { level: 2, labelKey: "auth.signup.strengthFair", color: "#f59e0b" };
  if (score === 3) return { level: 3, labelKey: "auth.signup.strengthGood", color: "#3b82f6" };
  return { level: 4, labelKey: "auth.signup.strengthStrong", color: "#22c55e" };
}
    ...
    return (
      <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "var(--bg)", fontFamily: "var(--font-body)", padding: 16 }}>
        <div style={{ width: "100%", maxWidth: 400, background: "var(--surface)", borderRadius: "var(--radius-xl)", padding: "48px 32px", boxShadow: "0 4px 32px rgba(0,0,0,0.08)", border: "1px solid var(--border)", textAlign: "center" }}>
          ...
          <h1 style={{ fontWeight: 700, fontSize: "var(--fs-xl)", marginBottom: 10 }}>{t("auth.confirm.title")}</h1>
          <p style={{ color: "var(--text-muted)", fontSize: "var(--fs-md)", lineHeight: 1.6, marginBottom: 24 }}>
            {t("auth.confirm.bodyBefore")}<strong style={{ color: "var(--text)" }}>{email}</strong>{t("auth.confirm.bodyAfter")}
          </p>
          <p style={{ fontSize: "var(--fs-sm)", color: "var(--text-muted)" }}>
            {t("auth.confirm.wrongAddress")}{" "}
    ...
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "var(--bg)", fontFamily: "var(--font-body)", padding: 16 }}>
      ...
      <div style={{ width: "100%", maxWidth: 400, background: "var(--surface)", borderRadius: "var(--radius-xl)", padding: "40px 32px", boxShadow: "0 4px 32px rgba(0,0,0,0.08)", border: "1px solid var(--border)" }}>
        <div style={{ textAlign: "center", marginBottom: 32 }}>
          ...
          <p style={{ color: "var(--text-muted)", fontSize: "var(--fs-sm)", margin: 0 }}>{t("auth.signup.subtitle")}</p>
        </div>
        ...
        <form onSubmit={handleSignup} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <div>
            <label style={{ fontSize: "var(--fs-sm)", fontWeight: 600, marginBottom: 6, display: "block", color: "var(--text)" }}>{t("auth.signup.fullName")}</label>
          ...
          <div>
            ...
            <input type="password" required value={password} onChange={e => setPassword(e.target.value)} placeholder={t("auth.signup.passwordHint")} minLength={6}
              style={{ width: "100%", padding: "12px 14px", border: "1.5px solid var(--border)", borderRadius: "var(--radius-md)", fontSize: "var(--fs-md)", outline: "none", fontFamily: "inherit" }} />
            {password.length > 0 && (
              <div style={{ marginTop: 8 }}>
                ...
                <div style={{ fontSize: "var(--fs-xs)", color: strength.color, fontWeight: 600 }}>{strength.labelKey ? t(strength.labelKey) : ""}</div>
              </div>
            )}
          </div>
          {error && (
          ...
          <p style={{ textAlign: "center", fontSize: "var(--fs-xs)", color: "var(--text-muted)", margin: 0, lineHeight: 1.5 }}>
            {t("auth.signup.agreeBefore")}
            <Link href="/terms" style={{ color: "var(--accent)", textDecoration: "none", fontWeight: 600 }}>{t("auth.signup.agreeTerms")}</Link>
            {t("auth.signup.agreeAnd")}
            <Link href="/privacy" style={{ color: "var(--accent)", textDecoration: "none", fontWeight: 600 }}>{t("auth.signup.agreePrivacy")}</Link>.
          </p>
        </form>

        <p style={{ textAlign: "center", marginTop: 20, fontSize: "var(--fs-sm)", color: "var(--text-muted)" }}>
          {t("auth.signup.haveAccount")}{" "}
          <Link href="/login" style={{ color: "var(--accent)", fontWeight: 700, textDecoration: "none" }}>{t("landing.nav.login")}</Link>
        </p>
        <p style={{ textAlign: "center", marginTop: 12, fontSize: "var(--fs-xs)", color: "var(--text-muted)" }}>
          {t("auth.signup.priceNote")}
        </p>
      </div>
    </div>
  );
